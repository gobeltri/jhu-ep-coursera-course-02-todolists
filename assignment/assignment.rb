...
class Assignment
  ...
  def create_todolist(params)
      ...
      # return an instance of the class with primary key (`id`), and dates (`created_at` and `updated_at`) assigned
      tl = TodoList.new
      tl.list_name = params[:name]
      tl.list_due_date = params[:due_date]
      tl.save!
  end
  ...
  def find_allusers(offset, limit)
      ...
      users = User.offset(offset).limit(limit).all.order(updated_at: :asc)
  end
  ...
  def find_alllists(offset, limit)
      ...
      todo_lists = TodoList.offset(offset).limit(limit).all.order(list_due_date: :desc)
  end
